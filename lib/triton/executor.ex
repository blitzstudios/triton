defmodule Triton.Executor do
  ...
  # Cassandra's reply to a conditional write whose IF clause failed; a lost race, not a failure
  @not_applied "Your operation was not applied."
  ...
  def execute(query, options \\ []) do
    ...
    do
      ...
      _ = case result do
        {:error, %{message: @not_applied}} -> :noop
  ...
  defp execute_error({:ok, %Xandra.Page{} = page}) do
    case page |> Enum.to_list |> List.first do
      %{"[applied]" => applied} -> if applied, do: {:ok, :success}, else: {:error, %{message: @not_applied}}
      _ -> {:error, %{message: @not_applied}}
    end
  end
